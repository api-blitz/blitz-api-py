...
def test_insufficient_credits_alias_is_gone() -> None:
    # Removed in 4.0.0 on the schedule set when it was deprecated in 2.3. Pinned so the
    # alias is not reintroduced by accident; the 402 class is InsufficientRecordsError.
    import blitz_api

    assert not hasattr(blitz_api, "InsufficientCreditsError")
    assert "InsufficientCreditsError" not in blitz_api.__all__
